...
const registerToFusionauth = asyncHandler(async (req, res, next) => {
  client
    ...
    .catch((error) => {
      console.log(error)
      next({ message: 'Registration Failed' })
    })
})

const loginToFusionauth = asyncHandler(async (req, res, next) => {
  if (req.session.user) {
    ...
    client
      ...
      .catch((error) => {
        ...
        next({ message: 'Login Failed, please check your credentials' })
      })
  }
})
