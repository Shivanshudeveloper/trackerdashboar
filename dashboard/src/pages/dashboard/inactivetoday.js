...
const InActiveToday = () => {
    ...
    if (user !== null) {
      try {
        const users = await axios.get(
          `${API_SERVICE}/api/activeUser/inActive/${user.organization}`
        );
  ...
  return (
    <>
      <Head>
        <title>Dashboard / InActive Today</title>
      </Head>
      <Box
        component="main"
        sx={{
...
InActiveToday.getLayout = (page) => <DashboardLayout>{page}</DashboardLayout>;

export default InActiveToday;
